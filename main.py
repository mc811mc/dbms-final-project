...
def main():
    ...
    # For column renaming in dataframe in pandas
...
def predictionError(model, X_test, y_test):
    ...
    print("----------- Predicting pollutant values with the model ------------")
    y_predicted = model.predict(X_test)
    # print(y_predicted)
    # print(y_test)
    print("-----------------y_predicted_df-------------")
    ...
    error = {}
    print("-----------------RMSE error for each target columns---------------")
    ...
    # print(maxValues)
    # maxValues.to_csv('downloaded data/maxValues.csv')
    # minValues = y_test_df.min()
    # minValues.to_csv('downloaded data/minValues.csv')
    error_df = pd.DataFrame.from_dict(error, orient='index')
    error_df.to_csv('downloaded data/error_df.csv')
    error_df = pd.read_csv('downloaded data/error_df.csv', names = ["attr", "value"])
    error_df = error_df.drop([0])
    print(error_df)
    error_df.plot(x='attr', y='value', kind='bar')
    plt.show()
    return error_df
...
def makePrediction(dataframe):
    ...
    error.plot(x='attr', y='value', kind='bar')
    plt.show()
    # print(error)
    ...
    # maxValues=maxValues.drop(maxValues.columns[2], axis=1)
    # maxValues=maxValues.drop(maxValues.columns[3], axis=1)
    # print(maxValues)
